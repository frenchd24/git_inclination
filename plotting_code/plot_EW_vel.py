...
$Id:  plot_EW_vel.py, v 1.0 06/18/18

Plot EW as a function of velocity and dv
...
def main():
    ...
    plot_EW_vel_mean_plus = True
    plot_EW_vel_mean_plus_save = True
    
    # plot dopplar b parameter as a function of dv w/o splitting, add other sets if you want
    plot_EW_dv_mean_plus = True
    plot_EW_dv_mean_plus_save = True
...
#         color_group = color_yellow
        ...
        azimuths2 = []
        Lya_vs2 = []
        for w, r, i, b, l, maj, az, v in zip(Lya_Ws, R_virs, impacts, bs, ls, MajDiams, azimuths, Lya_vs):
            if float(w) <= maxEW:
                Lya_Ws2.append(w)
                R_virs2.append(r)
                impacts2.append(i)
        ...
        associated_Lya_vs2 = []
        for w, r, i, b, l, maj, az, v in zip(associated_Lya_Ws, associated_R_virs, associated_impacts, associated_bs, associated_ls, associated_MajDiams, associated_azimuths, associated_Lya_vs):
            if float(w) <= maxEW:
        ...
        associated_xs = np.array(associated_Lya_vs2)
        ...
        two_azimuths2 = []
        two_Lya_vs2 = []
        for w, r, i, b, l, maj, az, v in zip(two_Lya_Ws, two_R_virs, two_impacts, two_bs, two_ls, two_MajDiams, two_azimuths, two_Lya_vs):
            if float(w) <= maxEW:
        ...
        three_Lya_vs = L_two_plus['Lya_vs']
        ...
        three_azimuths2 = []
        three_Lya_vs2 = []
        for w, r, i, b, l, maj, az, v in zip(three_Lya_Ws, three_R_virs, three_impacts, three_bs, three_ls, three_MajDiams, three_azimuths, three_Lya_vs):
            if float(w) <= maxEW:
        ...
        three_xs = np.array(three_Lya_vs2)
        ...
        group_adjustedIncs2 = []
        group_Lya_vs2 = []
        for w, r, i, group, b, l, maj, az, inc, v in zip(group_Lya_Ws, group_R_virs, group_impacts, group_mems, group_bs, group_ls, group_MajDiams, group_azimuths, group_adjustedIncs, group_Lya_vs):
        ...
        group_xs = np.array(group_Lya_vs2)
...
#             label=r'$\rm Group ~Mean ~EW$')
        ...
        xlabel(r'$\rm V_{{Ly\alpha}}~[km s^{{-1}}]$')
        ylabel(r'$\rm Ew~ [m \AA]$')
...
#         xlim(0, 2.5)
        ...
        if plot_EW_vel_mean_plus_save:
            savefig('{0}/W(vel)_mean_binSize{1}_plus2_cut{2}.pdf'.format(saveDirectory, binSize, maxEW),format='pdf',bbox_inches='tight')
        else:
            show()
...
#         leg.get_frame().set_alpha(0.5)
        ...
        if plot_EW_dv_mean_plus_save:
            savefig('{0}/W(dv)_mean_binSize{1}_plus2_cut{2}.pdf'.format(saveDirectory, binSize, maxEW),format='pdf',bbox_inches='tight')
        else:
            show()
