...
$Id:  plot_b_likelihood.py, v 1.0 06/18/18

Plot dopplar b-parameter as a function of likelihood and summed likelihood
...
def main():
    # plot dopplar b parameter as a function of likelihood w/o splitting, add other sets if you want
    plot_b_likelihood_mean_plus = True
    plot_b_likelihood_mean_plus_save = True

    # plot dopplar b parameter as a function of summed likelihood w/o splitting, add other sets if you want
    plot_b_summed_likelihood_mean_plus = False
    plot_b_summed_likelihood_mean_plus_save = False
...
##########################################################################################
    ...
    if plot_b_likelihood_mean_plus:
...
#         xlim(0, 2.5)
        ...
        if plot_b_likelihood_mean_plus_save:
            savefig('{0}/b(likelihood)_mean_binSize{1}_plus1.pdf'.format(saveDirectory, binSize),format='pdf',bbox_inches='tight')
        else:
            show()
...
##########################################################################################
    ...
    if plot_b_summed_likelihood_mean_plus:
        fig = figure(figsize=(7.7,5.7))
        ax = fig.add_subplot(111)
...
##########################################################################################
        ...
        xlabel(r'$\rm \Sigma \small \mathcal{L}$')
        ylabel(r'$\rm b~ [km s^{{-1}}]$')
...
#         xlim(0, 1.5)
        ...
        if plot_b_summed_likelihood_mean_plus_save:
            savefig('{0}/b(summed_likelihood)_mean_binSize{1}_plus4.pdf'.format(saveDirectory, binSize),format='pdf',bbox_inches='tight')
        else:
            show()
