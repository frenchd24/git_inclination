...
#         ylim(0,1300)
        ...
        if plot_b_cdf_save:
            savefig('{0}/hist(b)_all6_bins{1}_1_min_maxEW_{2}_{3}.pdf'.format(saveDirectory, binSize, minEW, maxEW),format='pdf',bbox_inches='tight')
        else:
            show()
...
##########################################################################################
    ...
    if plot_b_hist:
        ...
        L_group_bs_cut = []
        for w, m, b in zip(L_group_Lya_Ws, L_group_mems, L_group_bs):
            if w <= maxEW and w >= minEW and int(m) >= 3:
                L_group_Lya_Ws_cut.append(w)
                L_group_bs_cut.append(b)
...
#         ylim(0,1300)
        ...
        if plot_b_hist_save:
            savefig('{0}/hist(b)_bins{1}_L_group3.pdf'.format(saveDirectory, binSize),format='pdf',bbox_inches='tight')
        else:
            show()
