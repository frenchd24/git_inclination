...
$Id:  plot_b_diam.py, v 1.0 06/18/18

Plot dopplar b-parameter as a function of diameter and R_vir
...
def main():
    ...
    plot_b_diam_mean_plus = True
    plot_b_diam_mean_plus_save = True

    # plot dopplar b parameter as a function of R_vir w/o splitting, add other sets if you want
    plot_b_Rvir_mean_plus = True
    plot_b_Rvir_mean_plus_save = True
...
#         color_group = color_yellow
        ...
        for w, r, i, b, l, maj in zip(Lya_Ws, R_virs, impacts, bs, ls, MajDiams):
            if float(w) <= maxEW:
                Lya_Ws2.append(w)
                R_virs2.append(r)
                impacts2.append(i)
        ...
        isolated_xs = np.array(MajDiams2)
        ...
        for w, r, i, b, l, maj in zip(associated_Lya_Ws, associated_R_virs, associated_impacts, associated_bs, associated_ls, associated_MajDiams):
            if float(w) <= maxEW:
        ...
        associated_xs = np.array(associated_MajDiams2)
        ...
        two_ls2 = []
        two_MajDiams2 = []
        for w, r, i, b, l, maj in zip(two_Lya_Ws, two_R_virs, two_impacts, two_bs, two_ls, two_MajDiams):
            if float(w) <= maxEW:
        ...
        three_ls2 = []
        three_MajDiams2 = []
        for w, r, i, b, l, maj in zip(three_Lya_Ws, three_R_virs, three_impacts, three_bs, three_ls, three_MajDiams):
            if float(w) <= maxEW:
        ...
        three_xs = np.array(three_MajDiams2)
        ...
        group_ls2 = []
        group_MajDiams2 = []
        for w, r, i, group, b, l, maj in zip(group_Lya_Ws, group_R_virs, group_impacts, group_mems, group_bs, group_ls, group_MajDiams):
        ...
        group_xs = np.array(group_MajDiams2)
...
#         xlim(0, 2.5)
        ...
        if plot_b_diam_mean_plus_save:
            savefig('{0}/b(diam)_mean_binSize{1}_plus4.pdf'.format(saveDirectory, binSize),format='pdf',bbox_inches='tight')
        else:
            show()
...
#         color_group = color_yellow
        ...
        # make EW cut if needed, some other shit
        Lya_Ws2 = []
        R_virs2 = []
        impacts2 = []
        bs2 = []
        ...
        associated_xs = np.array(associated_R_virs2)
        associated_ys = np.array(associated_bs2)
        ...
        three_xs = np.array(three_R_virs2)
        three_ys = np.array(three_bs2)
        ...
        group_xs = np.array(group_R_virs2)
        group_ys = np.array(group_bs2)
...
#         xlim(0, 1.5)
        ...
        if plot_b_Rvir_mean_plus_save:
            savefig('{0}/b(R_vir)_mean_binSize{1}_plus4.pdf'.format(saveDirectory, binSize),format='pdf',bbox_inches='tight')
        else:
            show()
