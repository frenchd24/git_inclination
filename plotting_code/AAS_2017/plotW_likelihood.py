...
$Id:  plotW_likelihood.py, v 5.7 01/07/18



Based on:
plotW_impact_final.py, v 5.7 12/09/16
...
##########################################################################################
##########################################################################################
    # plot equivalent width as a function of likelihood, split between
...
##########################################################################################
##########################################################################################
    # plot equivalent width as a function of likelihood
    # 
    # FOR INCLUDE = 1 ABSORBERS


    plotW_like_median_include1 = True
    save = True
    
    if plotW_like_median_include1:
        fig = figure(figsize=(7.7,5.7))
        ax = fig.add_subplot(111)
        ...
        for i,w,v,l in zip(impactList,lyaWList,virList,likeList):
            # check if all the values are good
            if isNumber(i) and isNumber(w) and isNumber(v) and isNumber(l):
                if i!=-99 and w!=-99 and v!=-99 and w <=maxW and l!=-99:
                    xVal = float(l)
                    yVal = float(w)
...
#         leg.get_frame().set_alpha(0.5)
        ...
        if save:
            savefig('{0}/W(like)_median_binSize{1}_virsep_include1_maxEnv{2}.pdf'.format(saveDirectory,binSize,maxEnv),format='pdf',bbox_inches='tight')
        else:
            show()
...
##########################################################################################
##########################################################################################
    # plot equivalent width as a function of custom likelihood
    # 
    # FOR INCLUDE = 1 ABSORBERS


    plotW_cusLike_median_include1 = True
    save = True
    
    if plotW_cusLike_median_include1:
        fig = figure(figsize=(7.7,5.7))
        ax = fig.add_subplot(111)
        ...
        for i,w,v,l in zip(impactList,lyaWList,virList,likem15List):
            # check if all the values are good
            if isNumber(i) and isNumber(w) and isNumber(v) and isNumber(l):
                if i!=-99 and w!=-99 and v!=-99 and w <=maxW and l!=-99:
                    xVal = float(l)
...
#         leg.get_frame().set_alpha(0.5)
        ...
        if save:
            savefig('{0}/W(like_cus)_median_binSize{1}_virsep_include1_maxEnv{2}.pdf'.format(saveDirectory,binSize,maxEnv),format='pdf',bbox_inches='tight')
        else:
            show()
