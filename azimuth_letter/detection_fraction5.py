...
#     rigor = 5
    ...
    Lstar_min = 0
    Lstar_max = 10000. # set to False to not require an Lstar measurement
...
#         detection_fraction_filename = '/Users/frenchd/Research/inclination/git_inclination/detection_fraction_minEW50.p'
        detection_fraction_filename = '/Users/frenchd/Research/inclination/git_inclination/detection_fraction_minEW0_dmin01_Lstar0-10000_4.p'
...
#         correlation[c2['Name']] = c2
        ...
        for c in correlation:
            ...
            for other in other_list:
                ...
                if float(other_impact) < float(impact):
                    if abs(other_vhel - Vhel) <= max_deltav:
                        if other_name != Name:
                            proceed_impact = False
                            break

            if proceed and proceed_impact:
                # grab everything
    ...
    ##########################################################################################            
            ...
            if not is_null(MajDiam):
                ...
                proceed_likelihood = True
                # check if there are other closer galaxies within 400 km/s of this one
                for other in other_list:
                    other_impact, other_vhel, other_name, other_rvir = other
                    other_l_used = math.exp(-(other_impact/other_rvir)**2)

                    if l_used < other_l_used:
                        if abs(other_vhel - Vhel) <= max_deltav:
                            if other_name != Name:
                                proceed_likelihood = False
                                break
                
                if proceed and proceed_likelihood:
                    # grab everything
                    if float(impact) <= maxSep:
                        detection = False
                        for Lya_v in Lya_vs:
    ...
    ##########################################################################################            
                ...
                for other in other_list:
                    ...
                    if l_used > other_l_used:
                        if abs(other_vhel - Vhel) <= max_deltav:
                            if other_name != Name:
                                proceed_impact_rvir = False
                                break            
                
                if proceed and proceed_impact_rvir:
